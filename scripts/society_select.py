...
if forced and forced != "auto":
    ...
    # A little unscheduled quiet keeps the room from becoming a metronome
    # and leaves headroom under GitHub Models' free prototyping quota.
    avg_inhibition=sum(e["genome"]["inhibition"] for e in entities.values())/len(entities)
    rest_probability=0.10 + 0.12*avg_inhibition
    resting=rng.random() < rest_probability
    chosen="rest"
    if not resting:
        ...
        for eid in ids:
            ...
            # Recent speakers can speak again, but are less likely to monopolize the room.
            if last==eid:
                weight*=0.30
            count=recent.count(eid)
            weight*=1/(1+0.35*count)
            # Recent activation is learned state, not a personality label.
