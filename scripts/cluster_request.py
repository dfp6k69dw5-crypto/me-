...
def parse_issue_request(body: str):
    ...
    if (project, task) not in ALLOWED:
        raise ValueError(f'unsupported shared job: {project}/{task}')
    try:
    ...
    if not isinstance(terms, list) or not (2 <= len(terms) <= 4):
        raise ValueError('discovery job requires 2-4 terms')
    cleaned = []
    for term in terms:
        ...
        if not (1 <= len(t) <= 100):
            raise ValueError('term length out of range')
        cleaned.append(t)
...
def main():
    ...
    if mode == 'issues':
        ...
        if workload not in {'montecarlo','primes','hashstorm'}:
            raise ValueError('unsupported manual workload')
