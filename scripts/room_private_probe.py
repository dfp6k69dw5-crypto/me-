...
def main() -> int:
    ...
    write(status)
    if not all(status[k] for k in ("perception_secret_present", "deliberation_secret_present", "expression_secret_present")):
        return 0
    ...
    try:
        ...
        for key in ("ROOM_PROMPT_PERCEPTION", "ROOM_PROMPT_DELIBERATION", "ROOM_PROMPT_EXPRESSION", "ROOM_NODE_PROMPT"):
            clean.pop(key, None)
        proc = subprocess.Popen([str(server), "-m", str(model), "--host", "127.0.0.1", "--port", "18080", "-c", "8192", "-np", "4"], env=clean, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        try:
            for _ in range(120):
            ...
            tests = [
                ("comprehension", "ROOM_PROMPT_PERCEPTION", base, "perception_status"),
                ("thought", "ROOM_PROMPT_DELIBERATION", {**base, "social_observation": {"participation": "DIRECT_ADDRESSEE", "relationship_events": []}}, "deliberation_status"),
                ("expression", "ROOM_PROMPT_EXPRESSION", {**base, "social_observation": {"participation": "DIRECT_ADDRESSEE", "relationship_events": []}, "deliberation": {"action": "DEEPEN", "preferred_partner": "mara", "topic_facet": "evidence", "mandatory_speech": True}}, "expression_status"),
            ]
