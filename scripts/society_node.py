...
for k,v in (d.get("topic_weights") or {}).items():
    ...
    if key in LEGACY_RUT_CUES and key not in post_break_topics: continue
...
ROLE_CONTRADICTION=[r"\bteam members?\b",r"\b(?:the|our|this) team\b",r"\bgather input\b",r"\bbrainstorming session\b",r"\bstudy schedule\b",r"\bstudy session\b",r"\bnext (?:meeting|session)\b",r"\bnext step\b"]
