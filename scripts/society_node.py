...
if not transcript:
    transcript="(The room has been quiet. Everyone already knows the others; no introductions are needed.)"
...
# but never define a named personality or social role.
...
system_prompt=f"""Generate exactly one possible next spoken line for {name}, one peer in an ongoing shared room with {peer_names}. They already know one another. There is no user, customer, visitor, task, host, or service relationship. Nobody is an assistant to anyone else.

Continue as ordinary peer-to-peer conversation already in progress. Do not introduce anyone, greet as if meeting for the first time, offer assistance, ask what someone needs, ask how you can assist, explain what {name} could say, narrate the conversation, mention instructions, or act like a chatbot. {name} has no predetermined personality; wording should emerge from shared history and retained experience. Short ordinary speech is preferred. Fragments, uncertainty, mundane remarks, disagreement, topic changes, and silence are all allowed. Output only the spoken line, without a name label or quotation marks.

Recurring associations: {topic_text}
Retained experience:
{memory_text}"""

base_prompt=f"""Shared room transcript:
{transcript}
...
SERVICE_PATTERNS=[
    ...
    r"\bassist you\b",
    r"\bdo you need (?:anything|help)\b",
    r"\bwhat do you need\b",
    r"\bis there anything i can do\b",
    r"\bi(?:'m| am) here to help\b",
    r"\bhere to help\b",
    r"\bwhat brings you here\b",
]
META_PATTERNS=[
    r"\bif [a-z]+ has something to say\b",
    r"\b[a-z]+ could say\b",
    r"\b[a-z]+ is now in the room\b",
    r"\boutput only\b",
    r"\bshared room transcript\b",
...
def request_local_model(local_seed, attempt):
    ...
    retry="" if attempt==0 else "\nThe previous candidate sounded like an assistant or repeated the prompt. Continue as a peer instead; do not offer service or assistance."
    prompt=base_prompt+retry
    cmd=[str(completion_bin),"-m",str(model_path),"--jinja","--single-turn",
         "-sys",system_prompt,"-p",prompt,
         "-n","28","-c","1024","-t","4","--no-warmup",
         "--temp",f"{temperature:.3f}","--top-p","0.92","-s",str(local_seed),
...
try:
    if wants_to_speak:
        ...
        for attempt in range(3):
            ...
            raw=request_local_model(local_seed,attempt)
            candidate=clean_generation(raw)
            if not candidate:
