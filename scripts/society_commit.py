...
META=[r"\bif [a-z]+ has something to say\b",r"\b[a-z]+ could say\b",r"\boutput only\b",r"\brecent room speech\b",r"\bprevious candidate\b",r"\btoo generic or repetitive\b",r"\btry another natural line\b",r"\bdiffer substantially from the first attempt\b",r"\bselected earlier memories\b",r"\bpersistent adult background\b"]
...
def exact_recent(text):
    n=norm(text)
    return bool(n) and any(n==norm(m.get("text","")) for m in conversation[-12:])
def recent_similarity(text): return max((jac(text,m.get("text","")) for m in conversation[-12:]),default=0.0)

# Natural short pauses remain possible, but the room cannot drift into hours of silence.
# After three consecutive silent society turns, the next active entity gets a hard
# continuation opportunity. The machinery never supplies dialogue; it can only select
# among lines actually generated by that entity's own nodes.
HARD_SILENCE=3
silent_before=max(0,int(state.get("silent_turns",0) or 0))
silence_pressure=max(0.0,min(1.0,silent_before/HARD_SILENCE))
repeat_limit=min(0.82,0.62+0.20*silence_pressure)
hard_continuation=silent_before>=HARD_SILENCE
...
required_votes=1 if hard_continuation else 2

if votes>=required_votes:
    scored=[]
    for p in speakers:
        text=p["text"]
        recent_sim=recent_similarity(text)
...
# Hard fallback: if three ordinary silent turns have already occurred and all normal
# candidates were filtered, use the least repetitive non-control candidate generated
# by the nodes. This keeps the pause bounded without injecting a topic or a sentence.
if hard_continuation and not chosen:
    emergency=[]
    for p in raw_parts:
        e=p.get("emergency_candidate") or {}
        text=str(e.get("text","") or "").strip()
        if not text or core_forbidden(text): continue
        sim=recent_similarity(text)
        emergency.append((sim,p,e))
    if emergency:
        sim,p,e=min(emergency,key=lambda x:x[0])
        chosen={"node":p.get("node",0),"text":e.get("text",""),"topics":e.get("topics") or [],"novelty":float(e.get("novelty",max(0,1-sim))),"salience":0.45}
        emergency_used=True

if chosen:
    text=str(chosen["text"]).strip()
    agreement={"spoke":votes,"total":3,"chosen_node":int(chosen.get("node",0))+1}
    ...
    # Only actual spoken language teaches development. Emergency continuation is
    # deliberately reinforced much less so a forced continuation cannot create a loop.
    tw=d.setdefault("topic_weights",{}); reinforcement_scale=0.25 if emergency_used else 1.0
    for topic in chosen.get("topics") or []:
        topic=re.sub(r"[^a-z0-9 _'-]","",str(topic).lower()).strip()[:40]
        if topic:
            inc=reinforcement_scale*(.08+.30*g["plasticity"])*(.55+.45*float(chosen.get("salience",.5)))*(.55+.45*g["reinforcement_sensitivity"])
...
mode=" emergency" if emergency_used else (" ceiling" if hard_continuation and chosen else "")
print(f"{name}: {'spoke' if chosen else 'silent'} ({votes}/3 valid nodes voted to speak{mode})")
